...
class ShowMeQueriesMiddleware(MiddlewareMixin):
    ...
    def _reset_stats(self):
        self.stats = {'request': {}, 'response': {}}
        self.times = {"slower": {"query": None, "time": 0}}
        for alias in self.dbs:
            self.stats['request'][alias] = {
                'writes': 0,
                'reads': 0,
                'total': 0,
            }
            self.stats['response'][alias] = {
                'writes': 0,
                'reads': 0,
                'total': 0,
            }
        ...
        for c in connections.all():
            for q in c.queries:
                if not self._ignore_sql(q):
                    if (
                        q.get('sql')
                        and self.READ_QUERY_REGEX.search(q['sql']) is not None
                    ):
                    ...
                    if float(q['time']) > self.slower['time']:
                        self.slower = {
                            "query": q['sql'],
                            "time": float(q['time']),
                        }
    ...
    def process_response(self, request, response):
        if settings.DEBUG and not self._ignore_request(request.path):
            ...
            if QC_SETTINGS['RESPONSE_HEADER'] is not None:
                response[
                    QC_SETTINGS['RESPONSE_HEADER']
                ] = self._calculate_num_queries()
    ...
    def _stats_table(self, which, path='', output=None):
        if output is None:
            if self.host:
                host_string = 'http://{0}{1}'.format(
                    self.host, self.request_path
                )
            else:
    ...
    def _slower_time(self):
        time_outputs = ''
        if self.slower['time'] > QC_SETTINGS['MAX_TIME_TO_CHECK_SLOWER']:
            time_outputs += "--------------------- SLOWER QUERY -------------------------------\n"
            time_outputs += self.red('Time: {:.4f}s\n'.format(self.slower['time']))
            time_outputs += self.slower['query']
            time_outputs += "\n------------------------------------------------------------------\n"
        return time_outputs
    ...
    def print_num_queries(self):
        ...
        if (
            ...
            sys.stderr.write(self._slower_time())
